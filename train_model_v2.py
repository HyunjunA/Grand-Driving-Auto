...
def main():
    ...
    for e in range(epochs):
        ...
        file_nums = list(range(1,num_files_train+1))
        ...
        file_nums_val = list(range(1,num_files_val+1))
        random.shuffle(file_nums_val)
        j = 0
        while j < len(file_nums_val):
            val_data = []

            # Load 5 files
            for file_num in file_nums_val[j:j+5]:
                file_path = os.path.join(val_data_dir,f"training_data-{file_num}.npy")
                file_data = np.load(file_path,allow_pickle=True)
        ...
        test_metrics = {'val_loss':test_metrics[0],'val_accuracy':test_metrics[1]}
        tensorboard.on_batch_end(batch_no,test_metrics)
        print(f'Train metrics after epoch {e}: {train_metrics} Test metrics: {test_metrics}')
        print('Model saved')
    # Print final metrics
